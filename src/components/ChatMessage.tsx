...
export function ChatMessage({ content, type, isLoading }: ChatMessageProps) {
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex-1 prose prose-sm max-w-none break-words prose-invert">
          {isLoading ? (
            <LoadingDots />
          ) : (
            <ReactMarkdown
              ...
              components={{
                code({ className, children, node, ...props }) {
                  ...
                  if (!isInline && match) {
                    ...
                    return (
                      <div className="relative my-4 rounded-lg overflow-hidden w-full max-w-[calc(100vw-2rem)] sm:max-w-[800px] bg-gray-800 border border-gray-700">
                        <div className="flex items-center justify-between px-3 sm:px-4 py-2 border-b border-gray-700 bg-gray-900">
                  ...
                  return (
                    <code
                      className={cn(
                        ...
                        type === 'user' ? 'bg-gray-900 text-gray-100' : 'bg-gray-800 text-gray-100'
                      )}
                      {...props}
                    >
                      {children}
                ...
                p: ({ children }) => (
                  <p className="mb-4 last:mb-0 whitespace-pre-wrap break-words text-gray-100">
                    {children}
                  </p>
                ),
                h1: ({ children }) => <h1 className="text-2xl font-bold mb-4 text-gray-100">{children}</h1>,
                h2: ({ children }) => <h2 className="text-xl font-bold mb-3 text-gray-100">{children}</h2>,
                h3: ({ children }) => <h3 className="text-lg font-bold mb-2 text-gray-100">{children}</h3>,
